...
$res_empresas = buscarEmpresaByRucAndCorreo($conexion, $ruc, $correo);
$cantidad_empresas = mysqli_num_rows($res_empresas);
if (!verificar_sesion($conexion)) {
  ...
  if($cantidad_empresas != 0){
      echo "<script>
            alert('No es posible guardar una empresa con ruc o correo electrónico existente!');
            window.history.back();
            </script>";
  }else {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($errorArchivo === 0) {
            ...
            $rutaDestino = '../../empresa/files/' .$ruc.$extension;
        ...
        $sql = "INSERT INTO `empresa`(`razon_social`, `ruc`, `correo_institucional`, `ubicacion`, `contacto`, `cargo` , `celular_telefono`, `estado`, `usuario`, `ruta_logo`,`password`)
            VALUES ('$nombre_empresa' ,'$ruc', '$correo', '$ubicacion', '$contacto', '$cargo', '$celular', 'Activo', '$correo', '$rutaDestino','$pass')";
