...
text/x-generic imprimir_certificado_estudios.php ( PHP script, UTF-8 Unicode text, with CRLF line terminators )
<?php
...
if (!verificar_sesion($conexion)) {
  ...
  if($egresado == "SI"){
    $not = getNotasEgresado($conexion, $dni, $id_programa);
  }else{
  ...
  while ($notas = mysqli_fetch_array($not)) {
      $documento .= '
          <tr>
              <td><font size="10">' . $notas['unidad_didactica'] . '</font></td>
